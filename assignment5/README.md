...
*We plot top 300 frequent words in movie reviews out of 46517 words. A few words have the dominant frequency than other words. `./images/2021-01-25_11-14-11.png`*
...
The most similar words for `crime` were `gangster` and `criminal`. In the case, it is more like a topic. Because `feds`, `bars`, `shootout`, `giancarlo` (actor of crime drama series) aren't not directly having same meaning as `crime` . It is not interchangeable or synonym.

In summary, the LDA model are able to learn movie topics as latent variabel and can sometimes learns distributed representation for words. But it doesn't always effective for all similar word pairs or synonym. Most of times needs to try out repeatedly for finding one good example, although some of them seems like pretty well distributed representations like word2vec or glove. But in term of computation, the cost for representation learning is expensive. One can quickly learn the representations by applying other representation-oriented algorithm. 
