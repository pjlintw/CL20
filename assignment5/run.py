...
if __name__ == "__main__":
    ...
    def vertical_topic(width, topic_index, document_length):
        ...
        """
        m = np.zeros((int(width), int(width)))
    ...
    def save_document_image(filename, doc, zoom=2):
        ...
        #sm.imsave(filename, np.kron(doc, zoom))
        imageio.imwrite(filename, np.kron(doc, zoom))
    ...
    def gen_document(word_dist, n_topics, vocab_size, length=DOCUMENT_LENGTH, alpha=0.1):
        ...
        for n in range(length):
            #print('multinomial', np.random.multinomial(2, theta, size=None))
...
"""
# (n_topic,)
theta ~ Dirichlet( [alpha] * n_topic )
# topic index
z ~ multinomial(theta)

# (n_topic, vocab_size): topic and word co-occur
dist_zv
